...
#   ./scripts/cyberscale.sh misp-init Re-initialize MISP (templates + API key)
...
wait_for_misp() {
    log "Waiting for MISP to be ready (up to 90s)..."
    ...
    warn "MISP did not become ready within 90 seconds. Run './scripts/cyberscale.sh misp-init' manually later."
    return 1
}

misp_init() {
    if wait_for_misp; then
        log "Initializing MISP (templates + API key)..."
...
cmd_start() {
    ...
    log "Starting databases (postgres, misp-db, misp-redis)..."
    docker compose up -d postgres misp-db misp-redis

    log "Waiting for databases to be healthy..."
    docker compose up -d --wait postgres misp-db misp-redis 2>/dev/null || sleep 10
    ...
    # Check if MISP needs initialization (first run)
    ...
    if [ "$MISP_KEY" = "changeme-run-misp-authkey-setup" ] || [ -z "$MISP_KEY" ]; then
        log "First run detected — initializing MISP..."
        misp_init
    else
        ok "MISP API key already configured."
    fi
...
cmd_stop() {
    ...
    docker compose stop misp
    docker compose stop misp-redis misp-db
...
cmd_test() {
    ...
    docker compose exec cyberscale-web python -m pytest entity/tests/ -v --ignore=entity/tests/test_misp_integration.py
...
cmd_reset() {
    warn "This will destroy all data (database, MISP, volumes)."
...
case "${1:-}" in
    ...
    *)
        ...
        echo "  misp-init  Re-initialize MISP (templates + API key)"
